...
def get_url_list(dp_object, segment_duration, bitrate, brcount):
    """
    Module to get the List of URLs
    """
    media = dp_object.video[bitrate]
    ...
    if FORMAT == 0:
        ...
        media.url_list = URL_LIST[int(brcount * len(URL_LIST) /
                                      len(dp_object.video)):int(
                                          (brcount * len(URL_LIST) /
                                           len(dp_object.video)) +
                                          len(URL_LIST) /
                                          len(dp_object.video))]
...
def read_mpd(mpd_file, dashplayback):
    ...
    if 'MPD' in get_tag_name(root.tag).upper():
        if MEDIA_PRESENTATION_DURATION in root.attrib:
            ...
            config_dash.JSON_HANDLE["video_metadata"][
                'playback_duration'] = dashplayback.playback_duration
    ...
    if "Period" in get_tag_name(root[0].tag):
        ...
        child_period = root[1]
        FORMAT = 1
    elif "Period" in get_tag_name(root[2].tag):
        child_period = root[2]
        FORMAT = 1
    #print child_period
    ...
    if FORMAT == 0:
        ...
        for adaptation_set in child_period:
            ...
            if 'mimeType' in adaptation_set.attrib:
                ...
                if media_found:
                    ...
                    config_dash.JSON_HANDLE["video_metadata"][
                        'available_bitrates'] = list()
                    for representation in adaptation_set:
                        bandwidth = int(representation.attrib['bandwidth'])
                        config_dash.JSON_HANDLE["video_metadata"][
                            'available_bitrates'].append(bandwidth)
                        media_object[bandwidth] = MediaObject()
                        media_object[bandwidth].segment_sizes = []
                        for segment_info in representation:
                            if "SegmentTemplate" in get_tag_name(
                                    segment_info.tag):
                                media_object[
                                    bandwidth].base_url = segment_info.attrib[
                                        'media']
                                media_object[bandwidth].start = int(
                                    segment_info.attrib['startNumber'])
                                media_object[bandwidth].timescale = float(
                                    segment_info.attrib['timescale'])
                                media_object[
                                    bandwidth].initialization = segment_info.attrib[
                                        'initialization']
                            if 'video' in adaptation_set.attrib['mimeType']:
                                if "SegmentSize" in get_tag_name(
                                        segment_info.tag):
                                    try:
                                        segment_size = float(
                                            segment_info.attrib['size']
                                        ) * float(SIZE_DICT[
                                            segment_info.attrib['scale']])
                                    except KeyError as e:
                                        config_dash.LOG.error(
                                            "Error in reading Segment sizes :{}"
                                            .format(e))
                                        continue
                                    media_object[
                                        bandwidth].segment_sizes.append(
                                            segment_size)
                                elif "SegmentTemplate" in get_tag_name(
                                        segment_info.tag):
                                    video_segment_duration = (
                                        float(segment_info.attrib['duration'])
                                        / float(
                                            segment_info.attrib['timescale']))
        ...
        config_dash.JSON_HANDLE["video_metadata"][
                        'available_bitrates'] = list()
        config_dash.JSON_HANDLE["video_metadata"][
                        'available_original_bitrates'] = list()
        ...
        for adaptation_set in child_period:
            for representation in adaptation_set:
                ...
                if media_found:
                    ...
                    bandwidth = int(representation.attrib['bandwidth'])
                    origbandwidth = int(representation.attrib['origbandwidth'])
                    # print(bandwidth,'yay\n\n')
                    # print(origbandwidth,'yay\n\n')
                    config_dash.JSON_HANDLE["video_metadata"][
                        'available_bitrates'].append(bandwidth)
                    config_dash.JSON_HANDLE["video_metadata"][
                        'available_original_bitrates'].append(origbandwidth)
                    media_object[bandwidth] = MediaObject()
                    media_object[bandwidth].segment_sizes = []
                    if 'startWithSAP' in adaptation_set.attrib:
                        media_object[bandwidth].start = int(
                            adaptation_set.attrib['startWithSAP'])
                    else:
                    ...
                    for segment_info in representation:
                        if "SegmentBase" in get_tag_name(segment_info.tag):
                            for init in segment_info:
                                media_object[
                                    bandwidth].initialization = cut_url + init.attrib[
                                        'sourceURL']

                        if 'video' in representation.attrib['mimeType']:
                            if "SegmentList" in get_tag_name(segment_info.tag):
                                config_dash.LOG.debug(
                                    "Segment Playback Duration = {}".format(
                                        video_segment_duration))

                                video_segment_duration = float(
                                    segment_info.attrib["duration"])
                                if "timescale" in segment_info.attrib:
                                    video_segment_duration /= float(
                                        segment_info.attrib["timescale"])

                                for segment in segment_info:
                                    if "Initialization" in get_tag_name(
                                            segment.tag):
                                        initialization = segment.attrib[
                                            "sourceURL"]
                                        media_object[
                                            bandwidth].initialization = initialization
                                    ...
                                    if "SegmentURL" in get_tag_name(segment.tag):
                                        ...
                                        if ',' in segment.attrib['media']: 
                                            listseg=[]
                                            for seg in segment.attrib['media'].split(','): listseg.append(cut_url+seg)
                                            URL_LIST.append(listseg)

...
def download_segment(segment_url, dash_folder, index_file=None):
    ...
    if isinstance(segment_url, list):
        # print('\n\n Yes \n\n')
        segment_download_time=0.0
        segment_decryption_time=0.0
        segment_size=0.0

        for seg in segment_url: #iterate over each tiled segment (seg) of segment_url if its a list
            parsed_uri = urllib.parse.urlparse(seg)
            ...
            while num_retries < 1:
                ...
                config_dash.LOG.info(" Starting download of the Tiled segment: {}, num_retries: {}".format(
                    seg, num_retries))

                bashCommand = "curl --retry 5 -o {} -s -w 'total_time=%{{{}}}\\nsize_download=%{{{}}}\\nhttp_code=%{{{}}}\\n' {}".format(
                    segment_filename, "time_total", "size_download", "http_code",
                    seg)
                process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                # print(segment_filename,'segment filename')
                # print(segment_filename.split('/')[-1],'after split segment_filename')

                output, error = process.communicate()

                config_dash.LOG.info(" Finished download of the Tiled segment: {}, num_retries: {}".format(
                    seg, num_retries))
                ...
                if not os.path.isfile(segment_filename):
                    config_dash.LOG.info(
                        "Basic-DASH: FileNotFoundError during Tiled segment download - {}. # retry = {}"
                        .format(seg, num_retries))
                    if num_retries == 5:
                ...
                if http_code != 200:
                    config_dash.LOG.info(
                        "Basic-DASH: ValueError during Tiled segment download; received http code = {}. # retry = {}"
            ...
            if "init" not in segment_filename:
                #print(segment_filename)
                bashCommand="time python3 decryptor.py "+str(segment_filename)
                result = subprocess.run(bashCommand, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)        
   
                ## UNIX standard ##
                # stderr_output = result.stderr.decode().split(' ')[2].split(':')
                # segment_decryption_time = float(stderr_output[0]*60) + float(stderr_output[1].split('e')[0])

                ## MAC standard ##
                stderr_output = result.stderr.decode().split('\n')[1].split('\t')[1].split('m')
                segment_decryption_time += float(stderr_output[0]*60) + float(stderr_output[1].split('s')[0])
    ...
    ###################################################################################################################
        ...
        while num_retries < 1:
            ...
            config_dash.LOG.info(" Starting download of the segment: {}, num_retries: {}".format(
                segment_url, num_retries))

            bashCommand = "curl --retry 5 -o {} -s -w 'total_time=%{{{}}}\\nsize_download=%{{{}}}\\nhttp_code=%{{{}}}\\n' {}".format(
                segment_filename, "time_total", "size_download", "http_code",
                segment_url)
            process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
            # print("ssssssstoooppppp")
            # print(segment_filename,'segment filename')
            # print(segment_filename.split('/')[-1],'after split segment_filename')

            output, error = process.communicate()

            config_dash.LOG.info(" Finished download of the segment: {}, num_retries: {}".format(
                segment_url, num_retries))
            # print("ssssssstoooppppp")
    ...
    return segment_size, segment_filename, segment_download_time, segment_decryption_time
...
def start_playback_smart(dp_object,
    ...
    orig_bitrates = config_dash.JSON_HANDLE["video_metadata"]['available_original_bitrates']
    orig_bitrates.sort()
    ...
    previous_segment_decrypt_times=[]
    ...
    for segment_number, segment in enumerate(
        ...
        if segment_number == dp_object.video[bitrate].start:
            ...
            if playback_type.upper() == "BASIC":
                current_bitrate, average_dwn_time = basic_dash2.basic_dash2(
                    ...
                    recent_download_sizes, [a + b for a, b in zip(previous_segment_times, previous_segment_decrypt_times)],
                    current_bitrate)
                ...
                current_bitrate, average_dwn_time = basic_dash.basic_dash(
                    segment_number, bitrates, average_dwn_time,
                    segment_download_time+segment_decryption_time, current_bitrate)
        ...
        if isinstance(segment_path, list): 
            segment_url=[]
            for seg in segment_path: segment_url.append(urllib.parse.urljoin(domain, seg))
        else: segment_url = urllib.parse.urljoin(domain, segment_path)
        #print "segment url"
        #print segment_url
        ...
        try:
            ...
            if not index_dict[str(int(current_bitrate))]["index_downloaded"]:
                ...
                _, index_segment_filename, _, _ = download_segment(
            ...
            segment_size, segment_filename, segment_download_time,segment_decryption_time = download_segment(
                segment_url,
                file_identifier,
                index_file=index_dict[str(int(current_bitrate))]["index_file"])
            config_dash.LOG.info("{}: Downloaded and Decrypted segment {}".format(
                playback_type.upper(), segment_url))
        except IOError as e:
            config_dash.LOG.error("Unable to save segment %s" % e)
            return None
        previous_segment_times.append(segment_download_time)
        previous_segment_decrypt_times.append(segment_decryption_time)
        recent_download_sizes.append(segment_size)
        # Updating the JSON information
        #print('\n\n yes \n\n',segment_url)

        if isinstance(segment_url, list): segment_name = segment_url[1].split('_')[-1]
        else: segment_name = os.path.split(segment_url)[1]
        #print('\n\n\n\n', segment_name, '\n\n\n')
        ...
        config_dash.JSON_HANDLE["segment_info"].append(
            (segment_name, current_bitrate, segment_size,
             segment_download_time,segment_decryption_time))
        ...
        if segment_number > config_dash.MAX_BUFFER_SIZE:
            ...
            orig_qoe = compute_qoe(bitrates=orig_bitrates,
                              my_quality=qualities[qoe_index - 1],
                              prev_quality=prev_quality,
                              rebuffer_time=rebuffer_time)
            orig_qoes.append(orig_qoe)


            config_dash.LOG.info("QoE for Segment # {}: Encrypted segment is {} and decrypted segment is {}".format(
                qoe_index, qoe, orig_qoe))
        ...
        config_dash.LOG.info(
            "Downloaded %s. Size = %s in %s seconds and decrypted in %s seconds" %
            (segment_url, segment_size, str(segment_download_time), str(segment_decryption_time) ))
        if previous_bitrate:
    ...
    df_cols = [
        ...
        "current_buffer_size", "quality", "bitrate", "original_bitrates", "interruption", "qoe", "original_qoe"
    ]
    unix_timestamps = []
    ...
    orig_bit_rates= [orig_bitrates[i] for i in qualities]

    segment_nums = [x for x in range(1, len(dp_list) + 1)]
    # print('\n\n\n\n',len(segment_nums))
    # print(len(unix_timestamps))
    # print(len(epoch_times))
    # print(len(current_playback_time))
    # print(len(current_buffer_sizes))
    # print(len(qualities))
    # print(len(bit_rates))
    # print(len(orig_bit_rates))
    # print(len(rebuffer_times))
    # print(len(qoes))
    # print(len(orig_qoes))
    ...
    df = pd.DataFrame({
        ...
        df_cols[5]: qualities,
        df_cols[6]: bit_rates,
        df_cols[7]: orig_bit_rates,
        df_cols[8]: rebuffer_times,
        df_cols[9]: qoes,
        df_cols[10]: orig_qoes
    })

    max_qoe = bitrates[-1] / 1000
    orig_max_qoe= orig_bitrates[-1] / 1000

    df["nqoe"] = df[df_cols[9]] / max_qoe
    config_dash.LOG.info("Average QoE = {}".format(df[df_cols[9]].mean()))
    config_dash.LOG.info("Average n-QoE = {}".format(df["nqoe"].mean()))

    df["original_nqoe"] = df[df_cols[10]] / orig_max_qoe
    config_dash.LOG.info("Average original QoE = {}".format(df[df_cols[10]].mean()))
    config_dash.LOG.info("Average original n-QoE = {}".format(df["original_nqoe"].mean()))
...
def main():
    ...
    # for ob in config_dash.JSON_HANDLE["video_metadata"]['available_original_bitrates']:
    #     print(ob,'ob')
